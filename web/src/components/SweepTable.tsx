...
export default function SweepTable() {
  const [selectedT, setSelectedT] = useState("2.00");
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full font-mono text-xs">
        <thead>
          <tr className="text-[10px] uppercase tracking-widest text-(--color-text-dark)">
            {["T", "ESC%", "ACC%", "COST RED%", "ACCEPT", "ESCAL"].map((h, i) => (
              <th
                key={h}
                className={`border-b border-(--color-border) pb-2.5 font-normal ${
        ...
        <tbody>
          {data.map((r) => {
            const sel = r.t === selectedT;
            return (
              <tr
                key={r.t}
                onClick={() => setSelectedT(r.t)}
              ...
              >
                <td className="border-b border-(--color-border) py-2.5 text-left">
                  <span className={sel ? "text-(--color-orange) font-semibold" : "text-(--color-text-dim)"}>
                    {r.t}
                  </span>
                  {sel && (
                    <span className="ml-2 rounded bg-(--color-orange) px-1 py-px text-[9px] font-semibold text-white">
                      SEL
                    </span>
                  )}
                </td>
                ...
                <td className={`border-b border-(--color-border) py-2.5 text-right ${r.acc >= 98 ? "text-(--color-green)" : "text-(--color-text-dark)"}`}>
                  {r.acc.toFixed(1)}
                </td>
                <td className={`border-b border-(--color-border) py-2.5 text-right ${sel ? "text-(--color-orange)" : "text-(--color-text)"}`}>
                  {r.cost.toFixed(1)}
                </td>
      ...
      <p className="mt-2 text-[11px] text-(--color-text-dark)">Click any row to explore threshold tradeoffs.</p>
    </div>
  );
}
