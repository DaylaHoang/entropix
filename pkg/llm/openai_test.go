...
func TestOpenAIClient_ProviderInErrors(t *testing.T) {
	...
	_, err := c.Complete(context.Background(), &api.ChatCompletionRequest{
		Model:    "auto",
...
func TestOpenAIClient_Complete(t *testing.T) {
	expected := api.ChatCompletionResponse{
		ID:      "chatcmpl-openai",
		...
		Choices: []api.Choice{
			{
				Index:   0,
				Message: &api.Message{Role: "assistant", Content: "Hello from OpenAI"},
			},
		},
	}
	...
	resp, err := c.Complete(context.Background(), &api.ChatCompletionRequest{
		Model:    "auto",
		Messages: []api.Message{{Role: "user", Content: "Hi"}},
	})
	if err != nil {
