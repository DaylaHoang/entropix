...
func applyDefaults(cfg *Config) {
	...
	if cfg.Cache.QdrantCollection == "" {
		cfg.Cache.QdrantCollection = "entropix_cache"
	}
	if cfg.Cache.StartupTimeoutSeconds == 0 {
		cfg.Cache.StartupTimeoutSeconds = 10
	}
	if cfg.Cache.InsertTimeoutSeconds == 0 {
		cfg.Cache.InsertTimeoutSeconds = 5
	}
}
...
func validate(cfg *Config) error {
	...
	if cfg.Entropy.Threshold <= 0 {
		return fmt.Errorf("analysis.threshold must be > 0, got %f", cfg.Entropy.Threshold)
	}
	if cfg.Entropy.WindowSize < 1 {
		return fmt.Errorf("analysis.window_size must be >= 1, got %d", cfg.Entropy.WindowSize)
	}
	if cfg.Entropy.TopLogprobs < 1 || cfg.Entropy.TopLogprobs > 20 {
		return fmt.Errorf("analysis.top_logprobs must be between 1 and 20, got %d", cfg.Entropy.TopLogprobs)
	}
	if cfg.Speculative.SoftThresholdMult <= 0 || cfg.Speculative.SoftThresholdMult >= 1.0 {
		return fmt.Errorf("parallel.soft_threshold_mult must be > 0 and < 1.0, got %f", cfg.Speculative.SoftThresholdMult)
	}
	if cfg.Drafter.Model == cfg.Heavyweight.Model && cfg.Drafter.BaseURL == cfg.Heavyweight.BaseURL {
		return fmt.Errorf("drafter.model and heavyweight.model must differ when using the same provider")
	}
	if cfg.Cache.IsEnabled() {
		...
		if cfg.Cache.EmbeddingModel == "" {
			return fmt.Errorf("cache.embedding_model is required when cache is enabled")
		}
		if cfg.Cache.QdrantCollection == "" {
			return fmt.Errorf("cache.qdrant_collection is required when cache is enabled")
		}
	}
	return nil
}
