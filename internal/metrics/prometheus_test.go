...
func TestPrometheusRecorder_RecordRequest(t *testing.T) {
	...
	if !strings.Contains(body, `entropix_requests_total{model="llama3-8b-8192",status="200"} 2`) {
		t.Errorf("expected 2 requests with status 200, got:\n%s", body)
	}
	if !strings.Contains(body, `entropix_requests_total{model="llama3-8b-8192",status="500"} 1`) {
...
func TestPrometheusRecorder_RecordUpstreamLatency(t *testing.T) {
	...
	if !strings.Contains(body, "entropix_upstream_latency_seconds") {
		t.Errorf("expected upstream latency metric, got:\n%s", body)
	}
	if !strings.Contains(body, `entropix_upstream_latency_seconds_count{provider="groq"} 2`) {
...
func TestPrometheusRecorder_RecordError(t *testing.T) {
	...
	if !strings.Contains(body, `entropix_errors_total{type="upstream_timeout"} 2`) {
		t.Errorf("expected 2 timeout errors, got:\n%s", body)
	}
	if !strings.Contains(body, `entropix_errors_total{type="invalid_request"} 1`) {
...
func TestPrometheusRecorder_RecordEntropy(t *testing.T) {
	...
	if !strings.Contains(body, "entropix_entropy_distribution") {
		t.Errorf("expected entropy_distribution metric, got:\n%s", body)
	}
	if !strings.Contains(body, `entropix_entropy_distribution_count 3`) {
...
func TestPrometheusRecorder_RecordRoutingDecision(t *testing.T) {
	...
	if !strings.Contains(body, `entropix_routing_decisions_total{decision="accept"} 2`) {
		t.Errorf("expected 2 accept decisions, got:\n%s", body)
	}
	if !strings.Contains(body, `entropix_routing_decisions_total{decision="escalate"} 1`) {
