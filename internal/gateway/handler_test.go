...
type mockLLMClient struct {
	completeResp *api.ChatCompletionResponse
	completeErr  error
	streamChunks []api.StreamChunk
	streamErr    error
}

func (m *mockLLMClient) Complete(_ context.Context, _ *api.ChatCompletionRequest) (*api.ChatCompletionResponse, error) {
	return m.completeResp, m.completeErr
}

func (m *mockLLMClient) Stream(_ context.Context, _ *api.ChatCompletionRequest) (<-chan llm.StreamChunk, error) {
	if m.streamErr != nil {
...
func newTestHandler(drafter, heavyweight *mockLLMClient) *chatHandler {
	...
	rtr := router.NewRouter(analysis.WindowConfig{
		Size:           cfg.WindowSize,
	...
	return newChatHandler(drafter, heavyweight, rtr, nil, &metrics.NoopRecorder{}, cfg, specCfg, nil, 5*time.Second)
}
...
func newTestHandlerWithSpec(drafter, heavyweight *mockLLMClient) *chatHandler {
	...
	return newChatHandler(drafter, heavyweight, rtr, exec, &metrics.NoopRecorder{}, cfg, specCfg, nil, 5*time.Second)
}
...
func newTestHandlerWithCache(drafter, heavyweight *mockLLMClient, cs cache.Store) *chatHandler {
	...
	disabled := false
	specCfg := config.SpeculativeConfig{Enabled: &disabled}
	return newChatHandler(drafter, heavyweight, rtr, nil, &metrics.NoopRecorder{}, cfg, specCfg, cs, 5*time.Second)
}
	...
	heavyweight := &mockLLMClient{
		completeResp: &api.ChatCompletionResponse{
			...
			Choices: []api.Choice{
				{
					Index:   0,
					Message: &api.Message{Role: "assistant", Content: "Heavyweight response"},
				},
			},
		},
	}
		...
		streamChunks: []api.StreamChunk{
			{
				...
				Choices: []api.Choice{{Index: 0, Delta: &api.Delta{Content: "Heavyweight"}}},
			},
			{
				ID: "hw-2", Object: "chat.completion.chunk", Model: "gpt-4o",
				Choices: []api.Choice{{Index: 0, Delta: &api.Delta{Content: " stream"}}},
			},
		},
	}
...
func TestHandler_SpeculativeEscalation(t *testing.T) {
	...
	heavyweight := &mockLLMClient{
		completeResp: &api.ChatCompletionResponse{
			...
			Choices: []api.Choice{
				{Index: 0, Message: &api.Message{Role: "assistant", Content: "Heavyweight response"}},
			},
		},
		streamChunks: []api.StreamChunk{
			{ID: "hw-1", Object: "chat.completion.chunk", Model: "gpt-4o",
				Choices: []api.Choice{{Index: 0, Delta: &api.Delta{Content: "Speculative"}}}},
			{ID: "hw-2", Object: "chat.completion.chunk", Model: "gpt-4o",
...
func TestHandler_UpstreamTimeout(t *testing.T) {
	drafter := &mockLLMClient{
		streamErr: &llm.UpstreamTimeoutError{Provider: "groq"},
	}
	heavyweight := &mockLLMClient{}
...
type mockCacheStore struct {
	lookupResp *api.ChatCompletionResponse
	lookupErr  error
	inserted   bool
	insertErr  error
	evicted    bool
...
func (m *mockCacheStore) Lookup(_ context.Context, _ []api.Message) (*api.ChatCompletionResponse, error) {
	return m.lookupResp, m.lookupErr
}

func (m *mockCacheStore) Insert(_ context.Context, _ []api.Message, _ *api.ChatCompletionResponse) error {
	m.inserted = true
	return m.insertErr
}
...
func TestHandler_CacheHit(t *testing.T) {
	cachedResp := &api.ChatCompletionResponse{
		ID: "cached-1", Object: "chat.completion", Model: "gpt-4.1-nano",
		Choices: []api.Choice{
			{Index: 0, Message: &api.Message{Role: "assistant", Content: "cached answer"}},
		},
	}
...
func TestHandler_CacheHitStream(t *testing.T) {
	cachedResp := &api.ChatCompletionResponse{
		ID: "cached-1", Object: "chat.completion", Model: "gpt-4.1-nano",
		Choices: []api.Choice{
			{Index: 0, Message: &api.Message{Role: "assistant", Content: "cached stream"}},
		},
	}
	cs := &mockCacheStore{lookupResp: cachedResp}
	handler := newTestHandlerWithCache(&mockLLMClient{}, &mockLLMClient{}, cs)
