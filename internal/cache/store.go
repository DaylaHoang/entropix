...
func buildKeyText(messages []api.Message) string {
...
func (s *VectorStore) Lookup(ctx context.Context, messages []api.Message) (*api.ChatCompletionResponse, error) {
	start := time.Now()
...
func (s *VectorStore) Insert(ctx context.Context, messages []api.Message, resp *api.ChatCompletionResponse) error {
	keyText := buildKeyText(messages)
	vector, err := s.embedder.Embed(ctx, keyText)
	if err != nil {
