...
func TestJudgeEvaluate(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := api.ChatCompletionResponse{
			Choices: []api.Choice{
				{Message: &api.Message{Content: string(respBody)}},
			},
		}
	...
	c := llm.NewOpenAICompatibleClient(srv.URL, "test-key", "test-model", "test", 10*time.Second)
	judge := NewJudge(c)
...
func TestJudgeMarkdownFences(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := api.ChatCompletionResponse{
			Choices: []api.Choice{
				{Message: &api.Message{Content: judgeResp}},
			},
		}
		json.NewEncoder(w).Encode(resp)
	}))
...
func TestJudgeScoreThreshold(t *testing.T) {
	...
	for _, tt := range tests {
		...
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			resp := api.ChatCompletionResponse{
				Choices: []api.Choice{
					{Message: &api.Message{Content: string(judgeResp)}},
				},
			}
			json.NewEncoder(w).Encode(resp)
		}))
