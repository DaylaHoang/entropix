...
func NewCollector(drafter, heavyweight llm.LLMClient, judge *Judge, cfg CollectorConfig) *Collector {
	return &Collector{
		drafter:     drafter,
...
func (c *Collector) Collect(ctx context.Context, prompt dataset.Prompt) (*Record, error) {
	...
	draftReq := &api.ChatCompletionRequest{
		Messages:    messages,
		Logprobs:    &logprobs,
	...
	heavyReq := &api.ChatCompletionRequest{
		Messages:  messages,
		MaxTokens: &maxTokens,
	}
