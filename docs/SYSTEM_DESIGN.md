...
Entropix is a stateful reverse proxy that sits between clients and LLM providers. It reduces inference costs by 91.6% by routing requests through a fast, cheap drafter model first, analyzing the drafter's token-level confidence via Shannon entropy, and only escalating to expensive frontier models when the drafter demonstrates high uncertainty.
