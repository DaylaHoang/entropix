...
import (
	...
	"github.com/DaylaHoang/entropix/internal/gateway"
...
func main() {
	...
	drafter := llm.NewOpenAICompatibleClient(
		cfg.Drafter.BaseURL,
		openaiKey,
		cfg.Drafter.Model,
	...
	heavyweight := llm.NewOpenAICompatibleClient(
	...
	if cfg.Cache.IsEnabled() {
		...
		cacheStore, err = cache.New(
			...
			time.Duration(cfg.Cache.StartupTimeoutSeconds)*time.Second,
			rec,
		)
		if err != nil {
